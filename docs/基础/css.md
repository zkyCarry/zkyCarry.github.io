...
子类： flex 缩写： flex-grow 比例  flex-shrink flex-basis。默认： 1，1 0；  flex-grow：2站两倍 其他一倍， flex-shrink 是否缩放 0 不缩放。  self-algin。
justify-content 株洲对其， align-items 横轴堆积。 flex-direction： 设置主轴很轴。   flex-wrap：no-warp； wrap-revese。 
...
子类使用哪个网格： 编号网格线， 命名网格线， 命名网格区域 grid-template-areas。
...
23. 响应式设计： 移动端有限， @media 媒体查询， 设置meat  width：device-width， initial-scale=1； user-scalabel=no； <meta name="viewport" content="width:750, user-scalable=no" > 陌陌。
...
30，研究别人网站好的效果，想想怎么做。
31； hsl 色相 饱和度，明度 
32. 动画相关 transition ：属性，持续时间，线性(可以自定义，贝塞尔，steps阶梯函数)，等待时间。 过渡 tranfrom：矩阵，倾斜，旋转，缩放，位移。 tranlate。  @keyframe 
animation：xxx， 持续时间， 线性，等待时间。 
